...
#'
#' @examples
plot.krnel<-function(x, label=TRUE, label.cex=0.6, label.col="red", plot.bbox=T, bbox.col="blue", title=T, plot.radii=T, plot.ellipse=T, plot.ws.bbox=F, ws.bbox.col="pink", plot.pois=F, plot.ws.pois=F, poi.col="lightblue", poi.ws.col="lightblue", custom.col=NULL){
  if (title){
  ...
  if (!is.null(custom.col) & length(custom.col)!=nrow(x$features)) stop("custom.col should have the same length as the number of features")
  for (g in 1:length(x$contours)){
    if (is.null(custom.col)){
      ...
      polygon(x$contours[[g]][,1],x$params$h-x$contours[[g]][,2],col = custom.col[g], border = NA)
    }
  }
  if (label){
